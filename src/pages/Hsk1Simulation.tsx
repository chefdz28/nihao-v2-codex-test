...
import { loadHskResults, saveHskResult } from '@/lib/hskResults';
import { markCompleted } from '@/lib/studentProgress';


/** V2.2.1 /hsk1-simulation — HSK1 PRACTICE simulation (clearly not official) */
export default function Hsk1Simulation() {
  ...
  const [answers, setAnswers] = useState<(string | null)[]>(Array(HSK1_QUESTIONS.length).fill(null));
  const [secondsLeft, setSecondsLeft] = useState(HSK1_TIME_MINUTES * 60);
  const [reviewing, setReviewing] = useState(false);
  const startRef = useRef<number>(0);
  const timerRef = useRef<ReturnType<typeof setInterval> | null>(null);

  const q: HskQ = HSK1_QUESTIONS[index];
  const score = answers.filter((a, i) => a === HSK1_QUESTIONS[i].correct).length;
  ...
  const start = () => {
    setPhase('test');
    setIndex(0);
    setAnswers(Array(HSK1_QUESTIONS.length).fill(null));
    setSecondsLeft(HSK1_TIME_MINUTES * 60);
    setReviewing(false);
  ...
  const finish = () => {
    ...
    setAnswers(prev => {
      const finalScore = prev.filter((a, i) => a === HSK1_QUESTIONS[i].correct).length;
      const passed = (finalScore / HSK1_QUESTIONS.length) * 100 >= HSK1_PASS_PCT;
      const minutes = Math.round((Date.now() - startRef.current) / 60000);
      saveHskResult({ date: new Date().toISOString().slice(0, 10), score: finalScore, total: HSK1_QUESTIONS.length, passed, minutes });
      ...
      void markCompleted('quiz', 'hsk1-sim', finalScore);
      // wrong answers → mistake notebook
      prev.forEach((a, i) => {
        const qq = HSK1_QUESTIONS[i];
        if (a !== null && a !== qq.correct) {
          recordMistake({
            ...
            link: '/hsk1-simulation',
          });
        }
      });
      awardXP(`hsk1_sim:${new Date().toISOString().slice(0, 10)}`, 30); // once per day
      return prev;
    });
    setPhase('result');
  };
  ...
  // ---------------- intro ----------------
  if (phase === 'intro') {
    const past = loadHskResults();
    return (
      <div className="max-w-[640px] mx-auto px-6 py-10">
        {gateOpen && <AuthGate context={'hsk1_sim'} onClose={closeGate} />}
        <JsonLd id="hsk1-quiz" data={quizLd({ name: 'محاكاة تدريبية لاختبار HSK1', description: 'محاكاة HSK1 مجانية بالعربية: استماع وقراءة بمؤقت ونتيجة فورية. ليست اختباراً رسمياً.', path: '/hsk1-simulation', numQuestions: 40 })} />
        <JsonLd id="hsk1-breadcrumb" data={breadcrumbLd([{ name: 'الرئيسية', path: '/' }, { name: 'اختبارات HSK', path: '/hsk-tests' }, { name: 'HSK1', path: '/hsk1-simulation' }])} />
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="liquid-glass p-8 text-center">
          ...
          <h1 className="font-display font-black text-3xl text-white mb-2">{t('hsk.title')}</h1>
          ...
          <div className="flex justify-center gap-6 text-xs mb-6" style={{ color: 'var(--color-text-tertiary)' }}>
            ...
            <span>⏱ {HSK1_TIME_MINUTES} {t('hsk.minutes')}</span>
          </div>
          <div className="flex justify-center mb-5">
            <PinyinToggle mode={pinyinModeVal} onChange={setPinyinMode} hskLevel={1} compact />
          </div>
  ...
  // ---------------- result ----------------
  if (phase === 'result') {
    const pct = Math.round((score / HSK1_QUESTIONS.length) * 100);
    const passed = pct >= HSK1_PASS_PCT;
    return (
      <div className="max-w-[700px] mx-auto px-6 py-10">
        <motion.div initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} className="liquid-glass p-8 text-center mb-6">
          ...
          <p className="text-sm mb-1" style={{ color: 'var(--color-text-secondary)' }}>{score} / {HSK1_QUESTIONS.length} · {t('hsk.passMark')} {HSK1_PASS_PCT}%</p>
          <p className="text-xs text-[#f59e0b] mb-5">+30 XP</p>
              ...
              return (
                <div key={i} className={`liquid-glass p-3 rounded-xl border ${ok ? 'border-[#10b981]/20' : 'border-[#FF3333]/25'}`}>
                  ...
                  <p className="text-xs mt-1">
                    ...
                    {qq.pinyin && pinyinIsVisible(1) && <PinyinText inline className="ms-2">{qq.pinyin}</PinyinText>}
                  </p>
                </div>
              );
            })}
        ...
        <HskToolsNav exclude={['/hsk1-simulation']} />
      </div>
    );
  }
    ...
    <div className="max-w-[640px] mx-auto px-6 py-8">
      <div className="flex items-center justify-between mb-4">
        <span className="text-xs px-3 py-1.5 rounded-full bg-white/5" style={{ color: 'var(--color-text-tertiary)' }}>
          {q.part === 'listening' ? `🎧 ${t('hsk.listening')}` : `📖 ${t('hsk.reading')}`} · {index + 1}/{HSK1_QUESTIONS.length}
        </span>
      ...
      <div className="h-1.5 rounded-full bg-white/10 overflow-hidden mb-6">
        <div className="h-full bg-[#FF3333] rounded-full transition-all" style={{ width: `${((index + 1) / HSK1_QUESTIONS.length) * 100}%` }} />
      </div>
      ...
      <div className="liquid-glass p-6">
        ...
        <p className="text-[10px] text-center mt-3" style={{ color: 'var(--color-text-tertiary)' }}>
          {answers.filter(a => a !== null).length}/{HSK1_QUESTIONS.length} {t('hsk.answered')}
        </p>
      </div>
    </div>
  );
