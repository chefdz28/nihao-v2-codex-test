...
import { awardDailyXP, XP_REWARDS } from '@/lib/gamification';
  ...
  const handleAnswer = (optionId: string) => {
    ...
    if (optionId === q.correct_option_id) {
      ...
      const pick = (id: string) => {
        ...
        return o ? (o.text || o.textEn || o.textAr || id) : id;
      };
      recordMistake({
        source: 'quiz',
        question: q.question_en || q.question_ar,
        yourAnswer: pick(optionId),
        correctAnswer: pick(q.correct_option_id),
        explain_ar: q.question_ar,
        link: levelId ? `/quiz/${levelId}` : '/courses',
      });
    }
  };
    ...
    if (currentQuestion < questions.length - 1) {
      ...
      if (user && levelId) {
        ...
        try {
          ...
          // V2.1: XP per quiz (anti-dupe by lesson+date), perfect bonus
          awardDailyXP(`quiz:${levelId}`, XP_REWARDS.quiz_finish + (score === questions.length ? XP_REWARDS.quiz_perfect_bonus : 0));
