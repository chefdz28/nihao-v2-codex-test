...
import { HSK2_QUESTIONS, HSK2_PASS_PCT, HSK2_TIME_MINUTES, type Hsk2Q } from '@/data/hsk2sim';
...
/** V3.3 /hsk2-simulation — HSK2 PRACTICE simulation (clearly not official) */
export default function Hsk2Simulation() {
  ...
  const [answers, setAnswers] = useState<(string | null)[]>(Array(HSK2_QUESTIONS.length).fill(null));
  const [secondsLeft, setSecondsLeft] = useState(HSK2_TIME_MINUTES * 60);
  const [reviewing, setReviewing] = useState(false);
  const startRef = useRef<number>(0);
  const timerRef = useRef<ReturnType<typeof setInterval> | null>(null);

  const q: Hsk2Q = HSK2_QUESTIONS[index];
  const score = answers.filter((a, i) => a === HSK2_QUESTIONS[i].correct).length;

  useEffect(() => () => { if (timerRef.current) clearInterval(timerRef.current); }, []);

  const start = () => {
    trackEvent('hsk2_sim_start', { hsk_level: 2 });
    setPhase('test');
    setIndex(0);
    setAnswers(Array(HSK2_QUESTIONS.length).fill(null));
    setSecondsLeft(HSK2_TIME_MINUTES * 60);
    setReviewing(false);
  ...
  const finish = () => {
    ...
    setAnswers(prev => {
      const finalScore = prev.filter((a, i) => a === HSK2_QUESTIONS[i].correct).length;
      ...
      prev.forEach((a, i) => {
        ...
        if (a !== null && a !== qq.correct) {
          recordMistake({
            ...
            link: '/hsk2-simulation',
          });
        }
      });
      awardXP(`hsk2_sim:${new Date().toISOString().slice(0, 10)}`, 35); // once per day
      return prev;
    });
    setPhase('result');
  };
  ...
  // ---------------- intro ----------------
  if (phase === 'intro') {
    const past = loadHskResultsByLevel(2);
    return (
      <div className="max-w-[640px] mx-auto px-6 py-10">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="liquid-glass p-8 text-center">
          ...
          <p className={`text-sm mb-6 ${isAr ? 'font-arabic' : ''}`} dir={isAr ? 'rtl' : 'ltr'} style={{ color: 'var(--color-text-secondary)' }}>
            {isAr ? '36 سؤالاً بنمط HSK2: 18 استماعاً (اضغط للسماع) و18 قراءة. يمكنك التنقل بين الأسئلة وتغيير إجاباتك قبل التسليم.' : '36 questions in the style of HSK2: 18 listening (tap to hear) and 18 reading. Navigate freely and change answers before submitting.'}
          </p>
          <div className="flex justify-center gap-6 text-xs mb-6" style={{ color: 'var(--color-text-tertiary)' }}>
            <span>🎧 18 {t('hsk.listening')}</span>
            <span>📖 18 {t('hsk.reading')}</span>
            <span>⏱ {HSK2_TIME_MINUTES} {t('hsk.minutes')}</span>
          </div>
          <div className="flex justify-center mb-5">
            <PinyinToggle mode={pinyinModeVal} onChange={setPinyinMode} hskLevel={2} compact />
          </div>
  ...
  // ---------------- result ----------------
  if (phase === 'result') {
    const pct = Math.round((score / HSK2_QUESTIONS.length) * 100);
    const passed = pct >= HSK2_PASS_PCT;
    return (
      <div className="max-w-[700px] mx-auto px-6 py-10">
        <motion.div initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} className="liquid-glass p-8 text-center mb-6">
          ...
          <p className="text-sm mb-1" style={{ color: 'var(--color-text-secondary)' }}>{score} / {HSK2_QUESTIONS.length} · {t('hsk.passMark')} {HSK2_PASS_PCT}%</p>
          <p className="text-xs text-[#f59e0b] mb-5">+35 XP</p>
              ...
              return (
                <div key={i} className={`liquid-glass p-3 rounded-xl border ${ok ? 'border-[#10b981]/20' : 'border-[#FF3333]/25'}`}>
                  ...
                  <p className="text-xs mt-1">
                    ...
                    {qq.pinyin && pinyinIsVisible(2) && <PinyinText inline className="ms-2">{qq.pinyin}</PinyinText>}
                  </p>
                </div>
              );
            })}
    ...
    <div className="max-w-[640px] mx-auto px-6 py-8">
      <div className="flex items-center justify-between mb-4">
        <span className="text-xs px-3 py-1.5 rounded-full bg-white/5" style={{ color: 'var(--color-text-tertiary)' }}>
          {q.part === 'listening' ? `🎧 ${t('hsk.listening')}` : `📖 ${t('hsk.reading')}`} · {index + 1}/{HSK2_QUESTIONS.length}
        </span>
      ...
      <div className="h-1.5 rounded-full bg-white/10 overflow-hidden mb-6">
        <div className="h-full bg-[#FF3333] rounded-full transition-all" style={{ width: `${((index + 1) / HSK2_QUESTIONS.length) * 100}%` }} />
      </div>
      ...
      <div className="liquid-glass p-6">
        ...
        {q.chinese && (
          <div className="text-center mb-5">
            ...
            {pinyinIsVisible(2) && q.pinyin && <PinyinText className="text-sm mt-1 block">{q.pinyin}</PinyinText>}
          </div>
        )}
        <div className="space-y-2">
          {q.options.map(opt => (
            <button key={opt} onClick={() => answer(opt)}
              ...
              <PinyinAnswerOption option={opt} showPinyin={pinyinIsVisible(2)} />
            </button>
          ))}
        </div>
        ...
        <p className="text-[10px] text-center mt-3" style={{ color: 'var(--color-text-tertiary)' }}>
          {answers.filter(a => a !== null).length}/{HSK2_QUESTIONS.length} {t('hsk.answered')}
        </p>
      </div>
    </div>
  );
